...
describe("Login and Register test", () => {
  ...
  it("It should login with registered user", () => {
    ...
    browser.waitUntil(() => {
      return (
        browser.getUrl() === "http://localhost:4200/welcome/" + user.username
      );
    }, 1000);

    assert.equal(
      browser.getUrl(),
      "http://localhost:4200/welcome/" + user.username
    );
  });
});
...
describe("Create an account for second test user", () => {
  ...
  it("It should login with the second registered user", () => {
    ...
    assert.notEqual(
      pageUrl,
      "http://localhost:4200/welcome/" + username.getValue()
    );
  });
});
  ...
  it("It should post status with the second logged in user", () => {
    ...
    for (let i = 0; i <= 5; i++) {
      statusInput.setValue(
        "This is the test status number " + i + " for second user"
      );
      submitBtn.click();
    }
  });
