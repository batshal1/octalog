...
class PostComponent extends Component {
  ...
  componentDidMount() {
    ...
    PostDataService.retrievePost(username, this.state.id).then((response) =>
      this.setState({
  ...
  onSubmit(values) {
    ...
    if (this.state.id === -1) {
      PostDataService.createPost(username, todo).then(() => {
        this.props.refreshFeed();
        this.props.stompClient.send("/app/postStatus", {}, true);
      });
    } else {
      PostDataService.updatePost(username, this.state.id, todo).then(() => {
        this.props.refreshFeed();
        this.props.stompClient.send("/app/postStatus", {}, true);
      });
    }
                ...
                <div className="create-content">
                  <Field
                    ...
                    placeholder={
                      "Hey " +
                      this.props.username +
                      ", Have you been in an adventure lately? Share it with you friends."
                    }
                    onChange={this.handleChange}
                  />
                </div>
                <div className="create-tool">
                  <button className="btn btn-success btn-status" type="submit">
                    Post
                  </button>
                </div>
              </fieldset>
